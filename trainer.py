...
def reconstruction_loss(recon: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    peak_weight = float(getattr(config, "PEAK_LOSS_WEIGHT", 0.0))
    if peak_weight <= 0:
        return squared_error.mean()

    magnitude = target.detach().abs()
    scale = torch.clamp(magnitude.max(), min=1e-8)
    weights = 1.0 + peak_weight * (magnitude / scale)
